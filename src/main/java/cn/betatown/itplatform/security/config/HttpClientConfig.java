...
/**
 * @项目名称：wyait-manage
 * @包名：com.wyait.manage.config @类描述：
 * @创建人：wyait @创建时间：2018-01-11 9:13 @version：V1.0
 */
@Configuration
public class HttpClientConfig {
	...
	public HttpClientBuilder getHttpClientBuilder(
			@Qualifier("poolingClientConnectionManager") PoolingHttpClientConnectionManager httpClientConnectionManager) {
	 ...
	 * // 代理的host地址
	 * 
	 * @Value("${httpclient.config.proxyhost}") private String proxyHost;
	 * 
	 * // 代理的端口号
	 * 
	 * @Value("${httpclient.config.proxyPort}") private int proxyPort = 8080;
	 * 
	 *//**
		 * HttpClient不仅支持简单的直连、复杂的路由策略以及代理。 </br>
		 * HttpRoutePlanner是基于http上下文情况下，客户端到服务器的路由计算策略，一般没有代理的话，就不用设置这个东西。
		 * </br>
		 * 这里有一个很关键的概念—Route：在HttpClient中，一个Route指 运行环境机器->目标机器host的一条线路， </br>
		 * 也就是如果目标url的host是同一个，那么它们的route也是一样的
		 * 
		 * @return
		 *//*
		 * @Bean public DefaultProxyRoutePlanner defaultProxyRoutePlanner(){
		 * HttpHost proxy = new HttpHost(this.proxyHost, this.proxyPort); return
		 * new DefaultProxyRoutePlanner(proxy); }
		 */

	/*
	 * @Value("${httpclient.config.keepAliveTime}") private int keepAliveTime =
	 * 30;
	 * 
	 *//**
		 * 连接保持策略:长连接并不使用于所有的情况，尤其现在的系统，大都是部署在多台服务器上，且具有负载均衡的功能， </br>
		 * 如果我们在访问的时候，一直保持长连接，一旦那台服务器挂了，就会影响客户端， </br>
		 * 同时也不能充分的利用服务端的负载均衡的特性，反而短连接更有利一些，这些需要根据具体的需求来定，而不是一言概括。
		 * 
		 * @return
		 *//*
		 * @Bean("connectionKeepAliveStrategy") public
		 * ConnectionKeepAliveStrategy connectionKeepAliveStrategy() { return
		 * new ConnectionKeepAliveStrategy() {
		 * 
		 * public long getKeepAliveDuration(HttpResponse response, HttpContext
		 * context) { // Honor 'keep-alive' header HeaderElementIterator it =
		 * new BasicHeaderElementIterator(
		 * response.headerIterator(HTTP.CONN_KEEP_ALIVE)); while (it.hasNext())
		 * { HeaderElement he = it.nextElement(); String param = he.getName();
		 * String value = he.getValue(); if (value != null &&
		 * param.equalsIgnoreCase("timeout")) { try { return
		 * Long.parseLong(value) * 1000; } catch (NumberFormatException ignore)
		 * { } } } return 30 * 1000; } }; }
		 */
